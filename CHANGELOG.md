...
- Settings 新增账号卡片视图设置，可在标准卡片与紧凑卡片之间切换；默认保持标准卡片，紧凑卡片减少来源文案、限制长账号名换行，并提升同屏可见账号数量。
- 官方 OpenAI 额度在紧凑卡片中改为 `5h@...` / `周@...` 的内联表达；标准卡片与“当前激活”卡片继续保留原有上下进度条布局。
- Settings 保存后会立即关闭设置窗口，并异步刷新主浮窗配置，不再等待额度/API 网络刷新。
- 托盘右键菜单的“快速选择账号/API”改为子菜单，账号/API 与精简使用情况移入二级菜单，避免一级菜单文本被截断。
- 小浮窗额度标签改为绑定实际刷新时间并补充 tooltip，避免固定“5h 额度 / 周额度”造成信息不一致。
